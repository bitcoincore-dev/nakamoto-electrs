...
fn electrum_scripthash_get_mempool_updates_on_deep_chain_replacement() {
    ...
    let mut grand_stream = TcpStream::connect_timeout(&local_addr, Duration::from_secs(5)).unwrap();
    grand_stream
        .set_read_timeout(Some(Duration::from_secs(5)))
        .unwrap();
    let mut grand_reader = BufReader::new(grand_stream.try_clone().unwrap());
    write!(
        grand_stream,
        r#"{{"jsonrpc":"2.0","id":1,"method":"blockchain.scripthash.subscribe","params":["{}"]}}"#,
    ...
    .unwrap();
    grand_stream.write_all(b"\n").unwrap();

    let mut line = String::new();
    grand_reader.read_line(&mut line).unwrap();
    let initial: serde_json::Value = serde_json::from_str(line.trim()).unwrap();
    assert!(initial["result"].is_null());
    ...
    for (id, tx) in [
        ...
        tx_stream.write_all(b"\n").unwrap();
        line.clear();
        tx_reader.read_line(&mut line).unwrap();
    }

    line.clear();
    while line.is_empty() {
        match grand_reader.read_line(&mut line) {
            Ok(0) => continue,
            Ok(_) => break,
            Err(err) if err.kind() == std::io::ErrorKind::WouldBlock => {
                thread::sleep(Duration::from_millis(50));
            }
            Err(err) => panic!("failed to read grandchild pending notification: {err}"),
        }
    }
    let grand_pending: serde_json::Value = serde_json::from_str(line.trim()).unwrap();
    assert!(grand_pending["result"].is_string());
    ...
    write!(
        repl_stream,
        r#"{{"jsonrpc":"2.0","id":5,"method":"blockchain.transaction.broadcast","params":["{}"]}}"#,
        hex::encode(bitcoin::consensus::encode::serialize(&replacement))
    )
    .unwrap();
    repl_stream.write_all(b"\n").unwrap();
    ...
    let grand_note: serde_json::Value = serde_json::from_str(line.trim()).unwrap();
    assert_eq!(grand_note["method"], "blockchain.scripthash.subscribe");
    assert_eq!(
        grand_note["params"][0],
        serde_json::json!(sh_of(&grandchild_script).to_hex())
    );
    assert!(grand_note["params"][1].is_null());
    ...
    let mempool_after: serde_json::Value = {
        ...
        write!(
            stream,
            r#"{{"jsonrpc":"2.0","id":6,"method":"blockchain.scripthash.get_mempool","params":["{}"]}}"#,
            sh_of(&grandchild_script).to_hex()
        )
        .unwrap();
        stream.write_all(b"\n").unwrap();
